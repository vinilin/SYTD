...
__int64 CIni::GetInt64(LPCTSTR lpSection, LPCTSTR lpKey, __int64 nDefault, int nBase ) const
{
	TCHAR sz[DEF_PROFILE_NUM_LEN + 1] = _T("");
	GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
	__int64 value = nDefault;
	*sz == _T('\0') ? value = nDefault : sscanf(sz,"%lld",&value);
	return value;
}
...
BOOL CIni::WriteInt64(LPCTSTR lpSection, LPCTSTR lpKey, __int64 nValue, int nBase) const
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
	__Int64ToString(nValue, szValue, nBase);
	return WriteString(lpSection, lpKey, szValue);
}
...
void CIni::__Int64ToString(__int64 nNumber, LPTSTR lpBuffer, int nBase)
{
	switch (nBase)
	{
	case BASE_BINARY:
	...
	default:
		_stprintf(lpBuffer, _T("%lld"), nNumber);
		break;
	}	
}

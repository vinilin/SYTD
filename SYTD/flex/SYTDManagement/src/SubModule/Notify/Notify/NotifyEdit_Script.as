	...
	public function set subs(val:ArrayCollection):void
	{
		this._subs = val;
	}
	...
	private function OnSave(evt:MouseEvent):void
	{
		...
		if (this.__opKind=="ADD")
		{
			_remote.AddNotify(this._subCode,
		...
		{
			_remote.UpdateNotify(this.__notifyId,
							   this._subCode,
							   this.tbox_title.text,
							   this.iFrame.value,
							   pubStartTime,
	...
	private function OnSaveResult(evt:ResultEvent):void
	{
		...
		if (this.__opKind=="ADD")
		{
			result=_remote.AddNotify.lastResult as String;
		}
		else if(this.__opKind=="UPDATE")
		{
			result=_remote.UpdateNotify.lastResult as String;
		}
		if (result!=null)
		{
			if (result.substr(0,2).toUpperCase()=="OK")
			{
				if (this.__opKind=="ADD")
				{
					this.__notifyId=result.substr(2);
				}

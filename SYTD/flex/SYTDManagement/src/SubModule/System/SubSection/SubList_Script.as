// ActionScript file
	...
	import SubModule.System.SubSection.SubEdit;
	import SubModule.System.SubSection.SubSearch;
	...
	private var _search_condition_subName:String="";
	//-------------------------------------------
	...
	{
		if (parentApplication.checkFuncPopedom("015004002")){this.btnAdd.enabled=true;} else {this.btnAdd.enabled=false;}		
		if (parentApplication.checkFuncPopedom("015004003")){this.btnUpdate.enabled=true;} else {	this.btnUpdate.enabled=false; dg.removeEventListener(ListEvent.ITEM_DOUBLE_CLICK,OnDgItemDbClick);}
		if (parentApplication.checkFuncPopedom("015004004")){this.btnDelete.enabled=true;} else {this.btnDelete.enabled=false;}
	}*/
	...
	private function Search():void
	{
		...
		_remote.GetSubList(this._search_condition_subCode,
						   this._search_condition_subName,
						   this._pageSize,
						   this._pageIndex);
	...
	{
		_searchResult=_remote.GetSubList.lastResult as ArrayCollection;
	...
	{
		var pop:SubEdit=SubEdit(PopUpManager.createPopUp(this,SubEdit,true));
		pop.subId=this.dg.selectedItems[0].ID;
		pop.opKind="UPDATE";
	...
	{		
		...
		if (opKind=="ADD")
		{
			...
			Custom.System.Log.WriteLog("SubSection","增加","增加分站" + subName,parentApplication._userInfo.trueName,id);
		}
		else
		{
			...
			Custom.System.Log.WriteLog("SubSection","修改","修改分站" + subName,parentApplication._userInfo.trueName,id);
		}
	}
	...
	private function OnDeleteResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection=_remote.DeleteSub.lastResult as ArrayCollection;
		if (result!=null)
		{
			var message:String="";
			for(var i:int=0;i<result.length;i++)
			{
				if (result.source[i].result=="0")
				{
					for(var j:int=0;j<this._searchResult.length;j++)
					{
						...
						{
							Custom.System.Log.WriteLog("SubSection","删除","删除分站"+this._searchResult.source[j].SUBNAME,parentApplication._userInfo.trueName,this._searchResult.source[j].ID);
				...
				{
					message += "删除"+result.source[i].SUBNAME+""+result.source[i].result+"\r\n";
				}
			}
			if (message!="")
			{
	...
	{
		var pop:SubSearch=SubSearch(PopUpManager.createPopUp(this,SubSearch,true));
		pop.searchCallBack=searchCallBack;
		PopUpManager.centerPopUp(pop);
	}
	private function searchCallBack(subCode:String,subName:String):void
	{
		this._search_condition_subCode=subCode;
		this._search_condition_subName=subName;
		this._pageIndex=0;
		Search();
	}
	/////////////////分页事件////////////////////////////////

// ActionScript file
	...
	//-------------------------------------------
	//查询参数
	private var _search_condition_subCode:String="";
	private var _search_condition_ip:String="";
	//-------------------------------------------
	...
	{
		if (parentApplication.checkFuncPopedom("015005002")){this.btnAdd.enabled=true;} else {this.btnAdd.enabled=false;}		
		if (parentApplication.checkFuncPopedom("015005002")){this.btnUpdate.enabled=true;} else {	this.btnUpdate.enabled=false; dg.removeEventListener(ListEvent.ITEM_DOUBLE_CLICK,OnDgItemDbClick);}
		if (parentApplication.checkFuncPopedom("015005002")){this.btnDelete.enabled=true;} else {this.btnDelete.enabled=false;}
	}
	*/
	private function Search():void
	{
		...
		_remote.GetIPList(this._search_condition_subCode,
						   this._search_condition_ip,
						   this._pageSize,
						   this._pageIndex);
	...
	{
		_searchResult=_remote.GetIPList.lastResult as ArrayCollection;
	...
	{
		var pop:SubIpEdit=SubIpEdit(PopUpManager.createPopUp(this,SubIpEdit,true));
		pop.ipId=this.dg.selectedItems[0].ID;
		pop.opKind="UPDATE";
	...
	{		
		...
		if (opKind=="ADD")
		{
			...
			Custom.System.Log.WriteLog("SubIpDivision","增加","增加分站"+subName+"的IP段："+startIp+" "+endIp,parentApplication._userInfo.trueName,id);
		}
		else
		{
			...
			Custom.System.Log.WriteLog("SubIpDivision","修改","修改分站"+subName+"的IP段："+startIp+" "+endIp,parentApplication._userInfo.trueName,id);
		}
	}
	...
	private function AlertDeleteCloseHandler(evt:CloseEvent):void
	{
		if (Alert.YES==evt.detail)
		{
			...
			for(var i:int=0;i<this.dg.selectedItems.length;i++)
			{
				...
				obj.STARTIP=this.dg.selectedItems[i].STARTIP;
				obj.ENDIP=this.dg.selectedItems[i].ENDIP;
				selectSrc.addItem(obj);
			}
			_remote=new RemoteObject("fluorine");
			_remote.source="ManagementService.Sys.SubIp";
			_remote.showBusyCursor=true;
			_remote.DeleteIp(selectSrc);
	...
	private function OnDeleteResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection=_remote.DeleteIp.lastResult as ArrayCollection;
		if (result!=null)
		{
			var message:String="";
			for(var i:int=0;i<result.length;i++)
			{
				if (result.source[i].result=="0")
				{
					for(var j:int=0;j<this._searchResult.length;j++)
					{
						...
						{
							Custom.System.Log.WriteLog("SubIpDivision","删除","删除分站"+result.source[i].SUBNAME+"的IP段："+result.source[i].STARTIP+" "+result.source[i].ENDIP,parentApplication._userInfo.trueName,id);
				...
				{
					message += "删除"+result.source[i].SUBNAME+"的IP段"+result.source[i].STARTIP+ " " + result.source[i].ENDIP + ","+result.source[i].result;
				}
			}
			if (message!="")
			{
	...
	{
		var pop:SubIpSearch=SubIpSearch(PopUpManager.createPopUp(this,SubIpSearch,true));
		pop.searchCallBack=searchCallBack;
		PopUpManager.centerPopUp(pop);
	}
	private function searchCallBack(subCode:String,ip:String):void
	{
		this._search_condition_subCode=subCode;
		this._search_condition_ip=ip;
		this._pageIndex=0;
		Search();
	}
	/////////////////分页事件////////////////////////////////

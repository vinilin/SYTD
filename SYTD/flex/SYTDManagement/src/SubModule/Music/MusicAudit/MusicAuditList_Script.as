	...
	import SubModule.Music.Music.MusicEdit;
	import SubModule.Music.Music.MusicSearch;
	import SubModule.Music.MusicAudit.MusicAudit;
	import SubModule.Music.MusicAudit.MusicPublish;
	...
	{
		var pop:MusicPublish= MusicPublish(PopUpManager.createPopUp(this,MusicPublish,true));
	...
	{
		var pop:MusicEdit = MusicEdit(PopUpManager.createPopUp(this,MusicEdit,true));
		pop.ids = this.dg.selectedItems[0].ID;
		pop.opKind = "UPDATE";
	...
	private function OnAccept(evt:MouseEvent):void
	{
		...
		var pop:MusicAudit = MusicAudit(PopUpManager.createPopUp(this,MusicAudit,true));
	...
	{		
		...
		if (opKind=="ADD")
		{
			...
			Custom.System.Log.WriteLog("Music","增加","增加电影" + subName,parentApplication._userInfo.trueName,id);
		}
		else
		{
			...
			Custom.System.Log.WriteLog("Music","修改","修改电影" + subName,parentApplication._userInfo.trueName,id);
		}
	}
	...
	private function OnDeleteResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection=_remote.DeleteMusic.lastResult as ArrayCollection;
		if (result!=null)
		{
			var message:String = "";
			for(var i:int=0;i<result.length;i++)
			{
				if (result.source[i].result == "0")
				{
					for(var j:int=0;j<this._searchResult.length;j++)
					{
						...
						{
							Custom.System.Log.WriteLog("Music","删除","删除电影"+this._searchResult.source[j].TITLE,parentApplication._userInfo.trueName,this._searchResult.source[j].ID);
	...
	{
		var pop:MusicSearch = MusicSearch(PopUpManager.createPopUp(this,MusicSearch,true));

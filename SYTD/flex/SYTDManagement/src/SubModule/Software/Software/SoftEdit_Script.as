	...
	import SubModule.Software.Software.DefaultPic;
	import SubModule.Software.Software.FileSet;
	...
	private var _subCode:String ;  //软件不是本站点发布页不能编辑
	...
	private function OnGetKindFault(evt:FaultEvent):void
	{
		...
		Alert.show("网络或系统错误，获取软件分类失败"+evt.fault.faultString,"错误",Alert.OK);
	}
	private function GetInfo():void
	{
		...
		_remote.showBusyCursor =true;
		_remote.GetSoftwareInfoById(this._ids);
	...
	private function OnGetInfoResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection = _remote.GetSoftwareInfoById.lastResult as ArrayCollection;
		if (result!=null && result.source.length>0)
		{
			this.tbox_title.text = result.source[0].TITLE;
			this.tbox_title.enabled = false;
			this.tbox_version.text = result.source[0].VERSION;
			this.tbox_director.text = result.source[0].MANUFACTURER;
	...
	private function GetFileSet():void
	{
	...
	private function OnSave(evt:MouseEvent):void
	{
		...
		{
			Alert.okLabel = "确定";
			Alert.show("请填写软件名称。","提示",Alert.OK);
			return;
		}
		if (this.tbox_version.text == "")
		{
			Alert.okLabel = "确定";
			Alert.show("请填写版本","提示",Alert.OK);
			return;
		}
		if (this.tbox_director.text == "")
		{
			Alert.okLabel = "确定";
			Alert.show("请填写开发商","提示",Alert.OK);
			return;
		}
		...
		{
			Alert.okLabel = "确定";
			Alert.show("请选择软件类型。","提示",Alert.OK);
			return;
		}
		if (this._defaultPic == "")
		{
		...
		Alert.yesLabel = "是";
		Alert.noLabel = "否";
		Alert.show("数据保存后不能修改，传输任务即将开启，请确定你的软件文件是否正确，信息是否完整？","提示",Alert.YES|Alert.NO,this,Save);
	}	
	private function Save(evt:CloseEvent):void
	{
		if (Alert.YES == evt.detail)
		{
			...
			if (this._opKind == "ADD")
			{
				_remote.AddSoftware(parentApplication._userInfo.trueName,
			...
			else
			{
				...
				_remote.UpdateSoftware(this._ids,
								 this.tbox_title.text,
								 this.cbox_publishType.selectedItem.CODE,
								 this.tbox_issueDate.text,
								 this.tbox_brief.text,
	...
	private function OnSaveResult(evt:ResultEvent):void
	{
		...
		if (this._opKind == "ADD")
		{
			result = _remote.AddSoftware.lastResult as String;
		}
		else
		{
			result = _remote.UpdateSoftware.lastResult as String;
		}
		if (result!="")
		{

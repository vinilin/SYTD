	...
	import SubModule.Software.Software.SoftEdit;
	import SubModule.Software.Software.SoftSearch;
	import SubModule.Software.SoftwareAudit.SoftAuditList;
	import SubModule.Software.SoftwareAudit.SoftPublish;
	...
	public function OnPublish(evt:MouseEvent):void
	{
		...
		var pop:SoftPublish= SoftPublish(PopUpManager.createPopUp(this,SoftPublish,true));
	...
	{
		var pop:SoftEdit = SoftEdit(PopUpManager.createPopUp(this,SoftEdit,true));
		pop.ids = this.dg.selectedItems[0].ID;
		pop.opKind = "UPDATE";
	...
	private function OnAccept(evt:MouseEvent):void
	{
		...
		var pop:SoftAudit = SoftAudit(PopUpManager.createPopUp(this,SoftAudit,true));
	...
	{		
		...
		if (opKind=="ADD")
		{
			...
			Custom.System.Log.WriteLog("Soft","增加","增加电影" + subName,parentApplication._userInfo.trueName,id);
		}
		else
		{
			...
			Custom.System.Log.WriteLog("Soft","修改","修改电影" + subName,parentApplication._userInfo.trueName,id);
		}
	}
	...
	private function AlertDeleteCloseHandler(evt:CloseEvent):void
	{
		if (Alert.YES==evt.detail)
		{
			...
			_remote=new RemoteObject("fluorine");
			_remote.source="ManagementService.FileT.Soft";
			_remote.showBusyCursor=true;
			_remote.DeleteSoft(selectSrc);
	...
	private function OnDeleteResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection=_remote.DeleteSoft.lastResult as ArrayCollection;
		if (result!=null)
		{
			var message:String = "";
			for(var i:int=0;i<result.length;i++)
			{
				if (result.source[i].result == "0")
				{
					for(var j:int=0;j<this._searchResult.length;j++)
					{
						...
						{
							Custom.System.Log.WriteLog("Soft","删除","删除电影"+this._searchResult.source[j].TITLE,parentApplication._userInfo.trueName,this._searchResult.source[j].ID);
				...
				{
					message += "删除软件"+result.source[i].TITLE + ","+result.source[i].result + "\r\n";
				}
			}
			if (message!="")
			{
	...
	{
		var pop:SoftSearch = SoftSearch(PopUpManager.createPopUp(this,SoftSearch,true));

	...
	{		
		...
		if (opKind=="ADD")
		{
			...
			Custom.System.Log.WriteLog("Cartoon","增加","增加动画" + subName,parentApplication._userInfo.trueName,id);
		}
		else
		{
			...
			Custom.System.Log.WriteLog("Cartoon","修改","修改动画" + subName,parentApplication._userInfo.trueName,id);
		}
	}
			...
			for(var i:int=0;i<result.length;i++)
			{
				if (result.source[i].result == "0")
				{
					for(var j:int=0;j<this._searchResult.length;j++)
					{
						...
						{
							Custom.System.Log.WriteLog("Cartoon","删除","删除动画"+this._searchResult.source[j].TITLE,parentApplication._userInfo.trueName,this._searchResult.source[j].ID);
				...
				{
					message += "删除动画"+result.source[i].TITLE + ","+result.source[i].result + "\r\n";
				}
			}
			if (message!="")
			{

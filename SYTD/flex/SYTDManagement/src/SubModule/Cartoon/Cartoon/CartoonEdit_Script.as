	...
	import SubModule.Cartoon.Cartoon.DefaultPic;
	import SubModule.Cartoon.Cartoon.FileSet;
	...
	private function OnGetKindFault(evt:FaultEvent):void
	{
		...
		Alert.show("网络或系统错误，获取动画分类失败"+evt.fault.faultString,"错误",Alert.OK);
	}
	private function GetInfo():void
	{
		...
		_remote.showBusyCursor =true;
		_remote.GetCartoonInfoById(this._ids);
	...
	private function OnGetInfoResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection = _remote.GetCartoonInfoById.lastResult as ArrayCollection;
	...
	private function OnSave(evt:MouseEvent):void
	{
		...
		{
			Alert.okLabel = "确定";
			Alert.show("请填写动画名称。","提示",Alert.OK);
			return;
		}
		if (this.tbox_director.text == "")
		{
			Alert.okLabel = "确定";
			Alert.show("请填写作者。","提示",Alert.OK);
			return;
		}
		...
		{
			Alert.okLabel = "确定";
			Alert.show("请选择动画类型。","提示",Alert.OK);
			return;
		}
		if (this._defaultPic == "")
		{
		...
		Alert.yesLabel = "是";
		Alert.noLabel = "否";
		Alert.show("数据保存后不能修改，传输任务即将开启，请确定你的动画文件是否正确，信息是否完整？","提示",Alert.YES|Alert.NO,this,Save);
	}	
	private function Save(evt:CloseEvent):void
	{
		if (Alert.YES == evt.detail)
		{
			...
			if (this._opKind == "ADD")
			{
				_remote.AddCartoon(parentApplication._userInfo.trueName,
			...
			else
			{
				...
				_remote.UpdateCartoon(this._ids,
								 this.tbox_title.text,
								 this.cbox_publishType.selectedItem.CODE,
								 this.tbox_issueDate.text,
								 this.tbox_brief.text,
	...
	private function OnSaveResult(evt:ResultEvent):void
	{
		...
		if (this._opKind == "ADD")
		{
			result = _remote.AddCartoon.lastResult as String;
		}
		else
		{
			result = _remote.UpdateCartoon.lastResult as String;
		}
		if (result!="")
		{

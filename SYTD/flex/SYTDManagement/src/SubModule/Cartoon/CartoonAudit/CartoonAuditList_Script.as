	...
	import SubModule.Cartoon.Cartoon.CartoonEdit;
	import SubModule.Cartoon.Cartoon.CartoonSearch;
	import SubModule.Cartoon.CartoonAudit.CartoonAudit;
	import SubModule.Cartoon.CartoonAudit.CartoonPublish;
	...
	public function OnPublish(evt:MouseEvent):void
	{
		...
		{
			Alert.show("请选择数据后在进行操作。","提示",Alert.OK);
			return ;
		}
		var pop:CartoonPublish= CartoonPublish(PopUpManager.createPopUp(this,CartoonPublish,true));
	...
	{
		var pop:CartoonEdit = CartoonEdit(PopUpManager.createPopUp(this,CartoonEdit,true));
		pop.ids = this.dg.selectedItems[0].ID;
		pop.opKind = "UPDATE";
	...
	private function OnAccept(evt:MouseEvent):void
	{
		...
		var pop:CartoonAudit = CartoonAudit(PopUpManager.createPopUp(this,CartoonAudit,true));
	...
	{		
		...
		if (opKind=="ADD")
		{
			...
			Custom.System.Log.WriteLog("Cartoon","增加","增加电影" + subName,parentApplication._userInfo.trueName,id);
		}
		else
		{
			...
			Custom.System.Log.WriteLog("Cartoon","修改","修改电影" + subName,parentApplication._userInfo.trueName,id);
		}
	}
	...
	private function AlertDeleteCloseHandler(evt:CloseEvent):void
	{
		if (Alert.YES==evt.detail)
		{
			...
			_remote.source="ManagementService.FileT.Cartoon";
			_remote.showBusyCursor=true;
			_remote.DeleteCartoon(selectSrc);
	...
	private function OnDeleteResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection=_remote.DeleteCartoon.lastResult as ArrayCollection;
		if (result!=null)
		{
			var message:String = "";
			for(var i:int=0;i<result.length;i++)
			{
				if (result.source[i].result == "0")
				{
					for(var j:int=0;j<this._searchResult.length;j++)
					{
						...
						{
							Custom.System.Log.WriteLog("Cartoon","删除","删除动漫"+this._searchResult.source[j].TITLE,parentApplication._userInfo.trueName,this._searchResult.source[j].ID);
				...
				{
					message += "删除动漫"+result.source[i].TITLE + ","+result.source[i].result + "\r\n";
				}
			}
			if (message!="")
			{
	...
	{
		var pop:CartoonSearch = CartoonSearch(PopUpManager.createPopUp(this,CartoonSearch,true));

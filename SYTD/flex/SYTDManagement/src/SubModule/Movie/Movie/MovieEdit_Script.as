	...
	import SubModule.Movie.Movie.DefaultPic;
	import SubModule.Movie.Movie.FileSet;
	...
	private function OnGetKindFault(evt:FaultEvent):void
	{
		...
		Alert.show("网络或系统错误，获取电影分类失败"+evt.fault.faultString,"错误",Alert.OK);
	}
	private function GetInfo():void
	{
		...
		_remote.showBusyCursor =true;
		_remote.GetMovieInfoById(this._ids);
	...
	private function OnGetInfoResult(evt:ResultEvent):void
	{
		...
		var result:ArrayCollection = _remote.GetMovieInfoById.lastResult as ArrayCollection;
		if (result!=null && result.source.length>0)
		{
			this.tbox_title.text = result.source[0].TITLE;
			this.tbox_title.enabled = false;
			this.tbox_player.text = result.source[0].PlAYER;
			this.tbox_director.text = result.source[0].DIRECTOR;
	...
	private function OnSave(evt:MouseEvent):void
	{
		...
		{
			Alert.okLabel = "确定";
			Alert.show("请填写电影名称。","提示",Alert.OK);
			return;
		}
	...
	private function Save(evt:CloseEvent):void
	{
		if (Alert.YES == evt.detail)
		{
			...
			if (this._opKind == "ADD")
			{
				_remote.AddMovie(parentApplication._userInfo.trueName,
	...
	private function OnSaveResult(evt:ResultEvent):void
	{
		...
		if (this._opKind == "ADD")
		{
			result = _remote.AddMovie.lastResult as String;
		}
		else
		{
			result = _remote.UpdateMovie.lastResult as String;
		}
		if (result!="")
		{

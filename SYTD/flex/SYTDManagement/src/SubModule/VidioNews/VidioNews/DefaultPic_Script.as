	...
	private function OnSelectFile(evt:MouseEvent):void
	{
		...
		var imageTypes:FileFilter = new FileFilter("Vidios (*.wmv, *.avi, *.rmvb)", "*.wmv;*.avi;*.rmvb");
	...
	private function saveFileToDb():void
	{
		...
		_remote.VidioSave(this.__articleId,this.__picFileName);
	...
	private function OnSaveToDbResult(evt:ResultEvent):void
	{
		...
		var result:String=_remote.VidioSave.lastResult as String;
		if (result!=null)
		{
		...
		{
			Alert.okLabel = "确定";
			Alert.show("保存视频文件至数据库失败。","系统提示",Alert.OK);
		}
	}
